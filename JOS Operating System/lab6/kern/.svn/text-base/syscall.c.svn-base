...
int
sys_send_packet(void* addr, size_t size){
    return send_packet(addr,size);
}

int
sys_recv_packet(void* addr, size_t* size){
    return recv_packet(addr,size);
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
    ...
    case SYS_send_packet:
        r = sys_send_packet((void*)a1,a2);
	break;
    case SYS_recv_packet:
        r = sys_recv_packet((void*)a1,(size_t*)a2);
	break;
    default:
        return -E_INVAL;
    }
...
void syscallwrap(struct SysFrame *sf){
    ...
    /*if(sf->tf_regs.reg_eax == SYS_recv_packet){
    cprintf("reg_edx = %x\n",sf->tf_regs.reg_edx);
    cprintf("reg_ecx = %x\n",sf->tf_regs.reg_ecx); 
    cprintf("reg_ebx = %x\n",sf->tf_regs.reg_ebx);
    cprintf("reg_edi = %x\n",sf->tf_regs.reg_edi);
