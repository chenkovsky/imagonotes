...
send_data(struct http_request *req, int fd) {
	...
	size_t readsize;
	void* buf = NULL;
	if((r = fstat(fd,&statebuf))){
		return r;
	}
	size = statebuf.st_size;
	buf = malloc(size);
	//if(buf == NULL){
		//return;
	//}
...
#define MAX_DATA_SIZE 1518
	int leftsize;
	int offset = 0;
	for(leftsize = readsize; leftsize > MAX_DATA_SIZE;leftsize -= MAX_DATA_SIZE){
		send(req->sock, buf+offset, MAX_DATA_SIZE, 0);
		offset += MAX_DATA_SIZE;
	}
	if (send(req->sock, buf+offset, leftsize, 0)){// != readsize
		free(buf);
		return -1;
	}
	free(buf);
...
send_file(struct http_request *req) {
	...
	if(statebuf.st_isdir){
        ...
        cprintf("is dir:%d\n",statebuf.st_isdir);
	file_size = statebuf.st_size;
	//read(fd, void *buf, nbytes);
	...
	// LAB 6: Your code here.
	if((r = send_data(req,fd))){
            cprintf("httpd send data\n");
		return r;
	}
	close(fd);
	return 0;
...
umain() {
	...
	cprintf("httpd bind ok\n");
	/* Listen on the server socket */
	if (listen(serversock, MAXPENDING) < 0) {
		die("Failed to listen on server socket");
	}
	cprintf("httpd listen ok\n");
	while (1) {
