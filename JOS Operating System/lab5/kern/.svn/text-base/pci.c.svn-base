                 ...
                 struct pci_driver *list, struct pci_func *pcif)
{
    ...
    for (i = 0; list[i].attachfn; i++) {//search the driver list
        if (list[i].key1 == key1 && list[i].key2 == key2) {
            ...
            if (r > 0) {
                if(pci_debug)
                    cprintf("pci_attach_match: matched\n");
                return r;
            }

            if (r < 0) {
                cprintf("pci_attach_match: attaching %x.%x (%p): %e\n",
...
{//seemly read the configure to the f
         ...
         bar += bar_width)
    {
        ...
        int regnum = PCI_MAPREG_NUM(bar); /* extract the region number */
        uint32_t base, size;
