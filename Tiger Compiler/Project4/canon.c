...
static T_stm reorder(expRefList rlist) {
  ...
  /*      Temp_temp t = Temp_newtemp();
        *rlist->head=T_Temp(t);
        ...
        int offset = F_newSpillLocation();
        *rlist->head=T_Mem(T_Binop(T_plus, T_Temp(F_FP()), T_Const(offset)));
        return seq(hd.s, seq(T_Move(T_Mem(T_Binop(T_plus, T_Temp(F_FP()), T_Const(offset))), hd.e), s));
      }
    }
 }
