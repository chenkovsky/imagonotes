...
//?totally 6 register?
static string Convert_to_physical(Temp_temp r)
{
  if(r == F_FP())
  	 return "%ebp";
  else if(r == F_RV())
  	 return "%eax";
  else if(r == F_ScratchTemp())
  	 return "%edx";
  else if(r == F_AssemScratchTemp(0))
  	 return "%ecx";
  else if(r == F_AssemScratchTemp(1))
  	 return "%ebx";
  else if(r == F_SP())
  	 return "%esp";
  else
		   ...
		   AS_targets jumps)
{
  ...
  for(p = assem; p && *p != '\0'; p++)
    if (*p == '`')
      switch(*(++p)) {
      case 's': {int n = atoi(++p);
		 string s = Convert_to_physical(nthTemp(src, n));
		 strcpy(result+i, s);
		 i += strlen(s);
	       }
	break;
      case 'd': {int n = atoi(++p);
		 string s = Convert_to_physical(nthTemp(dst, n));
		 strcpy(result+i, s);
		 i += strlen(s);
	       }
	break;
...
void AS_print(FILE *out, AS_instr i)
{
  char r[200]; /* result */
  switch (i->kind) {
  case I_OPER:
    ...
    break;
  case I_LABEL:
    format(r, i->u.LABEL.assem, NULL, NULL, NULL); 
    fprintf(out, "%s\n", r); 
    /* i->u.LABEL->label); */
    break;
  case I_MOVE:
    format(r, i->u.MOVE.assem, i->u.MOVE.dst, i->u.MOVE.src, NULL);
    fprintf(out, "\t%s\n", r);
    break;
  }
}
